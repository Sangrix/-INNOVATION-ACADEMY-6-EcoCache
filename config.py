...
CHUNK_SIZE      = int(os.getenv("CHUNK_SIZE", "2000"))        # 청킹 적용 시 청크 크기
CHUNK_OVERLAP   = int(os.getenv("CHUNK_OVERLAP", "200"))      # 오버랩

# ── Qdrant ────────────────────────────────────────────────────────────────────
QDRANT_URL      = os.getenv("QDRANT_URL", "http://localhost:6333")
QDRANT_API_KEY  = os.getenv("QDRANT_API_KEY") or None
_qdrant_local_path = os.getenv("QDRANT_LOCAL_PATH", "").strip()
_default_qdrant_local_path = BASE_DIR / "qdrant_local"
QDRANT_LOCAL_PATH = (
    Path(_qdrant_local_path)
    if _qdrant_local_path
    else (_default_qdrant_local_path if _default_qdrant_local_path.exists() else None)
)
VECTOR_SIZE     = 1024
