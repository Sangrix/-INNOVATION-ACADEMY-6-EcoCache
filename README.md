...
`llm_generation` is already instrumented in `query.py`, but it is only
recorded when the LM Studio server is running and the query is executed with
`--generate`.

### 25-query batch evaluation (`test_queries.json`)

The branch was executed with the full 25-question evaluation set and logged to
