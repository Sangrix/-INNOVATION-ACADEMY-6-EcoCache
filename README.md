...
| 날짜 | 담당 | 작업 내용 |
|------|------|-----------|
| 2026-04-29 | 조희승 | RAG + Semantic Cache 통합 코드 작성, `/rag` 폴더 구조 구성 |
| 2026-04-29 | 김건후 | `carbon_monitor.py` 구현 (CodeCarbon + pynvml 계측) |
| 2026-04-29 | 최민상 | `carbon_optimizer.py` 구현 (CI 적응형 θ 알고리즘) |
| 2026-04-29 | 윤의섭 | `run_eval.py` 작성, 1차 평가셋 25개 (`test_queries.json`) 구성 |
| 2026-04-29 | 전체 | B1 / B2 / CIASC(α=0.25·0.50·1.00) 1차 실험 실행 |
| 2026-04-29 | 윤의섭 | GitHub PR #1 (`feat/eval-pipeline`) 제출 |
| 2026-05-05 | 전체 | 멘토 피드백 반영, 2주차 방향 확정 (RAG 웹사이트, 대시보드, 아이콘) |
| 2026-05-11 | 윤의섭 | 운영용 평가셋 80개로 확장 (`test_queries.json` v2, paraphrase 쌍 포함) |
...
`test_queries.json`의 80개 테스트 질문을 일괄 실행하고 성능을 측정합니다.
...
============================================================
 ...
 [카테고리별 Top-1 평균]
  exact_match               0.9512  (n=15)
  paraphrase                0.8734  (n=25)
  ...
```
...
| 카테고리 | 설명 | 개수 |
|----------|------|------|
| `exact_match` | QA pair와 거의 동일한 질문 | 15 |
| `paraphrase` | 같은 의미, 다른 표현 (캐시 히트 유도) | 25 |
| `specific_info` | 신청 방법·기간 등 세부 정보 질문 | 38 |
| `out_of_scope` | 데이터셋에 없는 토픽 (낮은 유사도 확인) | 2 |
...
├── test_queries.json       # 80개 운영용 평가셋 (SW중심대학 공지사항 기반, v2)
...
`llm_generation`도 [query.py](query.py)에
...
### 1주차 배치 평가 결과 — 25문항 (`test_queries.json` v1, 2026-04-29)

25개 테스트 질문 전체를 B1 / B2 / CIASC 모드로 실행했습니다.  
결과는 `examples/test_set_eval_log.jsonl`에 저장되어 있습니다.
...
### 2주차 운영용 평가셋 — 80문항 (`test_queries.json` v2, 2026-05-11)

기존 25개에서 80개로 확장한 운영용 평가셋입니다.  
캐시 히트 효과를 측정하기 위해 paraphrase 쌍을 대폭 추가했습니다.

| 카테고리 | 설명 | 개수 |
|----------|------|------|
| `exact_match` | QA pair와 거의 동일한 질문 | 15 |
| `paraphrase` | 같은 의미, 다른 표현 (캐시 히트 유도) | 25 |
| `specific_info` | 세부 정보 질문 (일정·대상·금액 등) | 38 |
| `out_of_scope` | 데이터 범위 밖 질문 (낮은 유사도 확인) | 2 |

> paraphrase 예시: `"TOPCIT 신청 기간이 언제인가요?"` / `"토픽시트 시험 언제 신청해요?"`
